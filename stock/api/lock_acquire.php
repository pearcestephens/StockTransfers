...
try {
    ...
    $svc->cleanup();
    $before = $svc->getLock($transferId);
    $result = $svc->acquire($transferId, $uid, $fingerprint);
    if(($result['success']??false) && isset($result['lock'])){
        $audit->lockAcquire($transferId, $uid, $before && (int)$before['user_id']===$uid);
        $resolver=new StaffNameResolver();
        $result['lock']['holder_name']=$resolver->name((int)$result['lock']['user_id']);
    } elseif(($result['conflict']??false) && isset($result['holder']['user_id'])) {
        $resolver=new StaffNameResolver();
        $result['holder']['holder_name']=$resolver->name((int)$result['holder']['user_id']);
    }
